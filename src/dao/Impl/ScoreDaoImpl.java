...
public class ScoreDaoImpl implements ScoreDao {
	...
	public List findAll() {
		...
		try {
			while (rs.next()) {
				...
				String scoreGrade = rs.getString("score_grade");
	...
	public PageBean scoreListPage(int pageNo, int pageCount, String stuNum) {
		...
		try {
			while (rs.next()) {
				...
				ScoreCou scoreCou = new ScoreCou(courseName, courseCredit,
						courseHours, courseTea, major, courseDate, scoreGrade,scoreId);
				list.add(scoreCou);
			}
	...
	public PageBean selectStuScore(int pageNo, int pageCount, Score score) {
		...
		if(score.getStuNum() != null){
			sql = "select * from score where stu_num like '%"
					+ score.getStuNum() + "%' limit " + ((pageNo - 1) * pageCount)
					+ "," + pageCount;
			sqlCount = "select count(*) from score where stu_num like '%"
					+ score.getStuNum() + "%'";
		}
		if(score.getStuName() != null){
			sql = "select * from score where stu_name like '%"
					+ score.getStuName() + "%' limit " + ((pageNo - 1) * pageCount)
					+ "," + pageCount;
			sqlCount = "select count(*) from score where stu_name like '%"
					+ score.getStuName() + "%'";
		}
		if(score.getCourseName() != null){
			sql = "select * from score where course_name like '%"
					+ score.getCourseName() + "%' limit " + ((pageNo - 1) * pageCount)
					+ "," + pageCount;
			sqlCount = "select count(*) from score where course_name like '%"
					+ score.getCourseName() + "%'";
		}
		if(score.getStuClass() != null){
			sql = "select * from score where stu_class like '%"
					+ score.getStuClass() + "%' limit " + ((pageNo - 1) * pageCount)
					+ "," + pageCount;
			sqlCount = "select count(*) from score where stu_class like '%"
					+ score.getStuClass() + "%'";
		}
		if(score.getMajor() != null){
			sql = "select * from score where major like '%"
					+ score.getStuClass() + "%' limit " + ((pageNo - 1) * pageCount)
					+ "," + pageCount;
			sqlCount = "select count(*) from score where major like '%"
					+ score.getMajor() + "%'";
		}
		ResultSet rs = dbCon.find(sql);
		try {
	...
	@Override
	public String SearchCredict(String name) {
       return " ";
	}
