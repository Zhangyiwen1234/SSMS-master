...
<form action="ServletSearchScore" method="post" id="frm">
	<div class="div_list">
		<%--课   名：<input type="text" class="form1 form-control"   name="cosName" value="${score.getCourseName() }" />&nbsp;&nbsp;--%>
		...
		<%--电话号码：<input type="text"  class="form1 form-control" name="stuClass" value="${score.getStuClass() }" />&nbsp;&nbsp;
		...
		<div class=" panel panel-default">
			...
			<table class="table panel-body" id="tableClass">
				...
				<c:forEach var="list" items="${list.pbjs }">
					<tr>
						<td><a href="ServletSelectScore?scoreID=${list.getScoreID() }">廉政证明</a><%--最后链接到廉政证明中去--%>
						...
						<td>${list.getMajor() }</td>
						<td><a href="ServletSelectScore?scoreID=${list.getScoreID() }">查看详情</a><%--到时候更换路径到另一个详情页面--%>
